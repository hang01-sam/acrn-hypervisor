...
gic_ops_t gicv3_ops = {
	.init = gicv3_init,
	.ack_irq = gicv3_acknowledge_interrupt,
	.raise_sgi = gicv3_raise_sgi_cpumask,
	.enable_irq = gicv3_enable_interrupt,
	.disable_irq = gicv3_disable_interrupt,
	.set_irq_pending = gicv3_set_interrupt_pending,
	.clear_irq_pending = gicv3_clear_interrupt_pending,
	.set_irq_cfg = gicv3_interrupt_set_cfg,
	.set_irq_priority = gicv3_set_interrupt_priority,
	.end_irq = gicv3_end_of_interrupt,
	.deactivate_irq = gicv3_deactivate_interrupt,
};

static uintptr_t rdistif_base_addrs[CONFIG_GICR_NUM];
static const gicv3_driver_data_t _gic_data = {
	.gicd_base = CONFIG_GICD_BASE,
	.gicr_base = CONFIG_GICR_BASE,
	.rdistif_base_addrs = rdistif_base_addrs,
	.rdistif_num = CONFIG_GICR_NUM,
};

void gic_pre_init(void)
{
	gic_register_ops(&gicv3_ops);
}

void gicv3_init(uint16_t pcpu_id)
{
	if (pcpu_id == 0) {
		gicv3_driver_init(&_gic_data);
		//gicv3_rdistif_probe(_gic_data.gicr_base);
		gicv3_distif_init();
		gicv3_rdistif_init(pcpu_id);
		gicv3_cpuif_enable();
	} else {
		gicv3_rdistif_init(pcpu_id);
		gicv3_cpuif_enable();
...
void gicv3_driver_init(const gicv3_driver_data_t *plat_driver_data)
{
	...
	pr_info("GICv%u with%s legacy support detected.", gic_version,
			(gicv2_compat == 0U) ? "" : "out");
	pr_info("ARM GICv%u driver initialized in EL2", gic_version);
}
...
void gicv3_distif_init(void)
{
	...
	gicd_clr_ctlr(gicv3_driver_data->gicd_base,
				  CTLR_ENABLE_G1NSA_BIT |
				  CTLR_ENABLE_G1NS_BIT,
				  RWP_TRUE);
	//gicd_write_ctlr(gicv3_driver_data->gicd_base, 0U);
	/* Set the default attribute of all (E)SPIs */
	gicv3_spis_config_defaults(gicv3_driver_data->gicd_base);
	/* Set the ARE_S and ARE_NS bit now that interrupts have been disabled */
	gicd_set_ctlr(gicv3_driver_data->gicd_base, CTLR_ARE_NS_BIT, RWP_TRUE);

	gicd_set_ctlr(gicv3_driver_data->gicd_base, CTLR_ENABLE_G1NS_BIT | CTLR_ENABLE_G1NSA_BIT, RWP_TRUE);
}
 ...
 ******************************************************************************/
void gicv3_cpuif_enable(void)
{
	uintptr_t gicr_base;
	unsigned int proc_num = get_pcpu_id();
	...
	write_icc_sre_el2(read_icc_sre_el2() | ICC_SRE_SRE_BIT);
	write_icc_igrpen1_el1(IGRPEN1_EL1_ENABLE_G1_BIT);
	write_icc_bpr1_el1(0);
	write_icc_ctlr_el1(ICC_CTLR_EOI_BIT);
	isb();
...
void gicv3_deactivate_interrupt(unsigned int id)
{
	write_icc_dir_el1(id);
}
 ...
 ******************************************************************************/
void gicv3_enable_interrupt(unsigned int id)
{
	uintptr_t gicd_base;
 ...
 * Redistributor interface.
 ******************************************************************************/
void gicv3_disable_interrupt(unsigned int id)
{
	uintptr_t gicd_base;
 ...
 * id.
 ******************************************************************************/
void gicv3_set_interrupt_priority(unsigned int id, unsigned int priority)
{
	uintptr_t gicr_base;
	uintptr_t gicd_base;
 ...
 ******************************************************************************/
void gicv3_raise_sgi(int sgi_num, u_register_t target)
{
	unsigned int tgt, aff3, aff2, aff1, aff0;
	uint64_t sgi_val;

	/* Verify interrupt number is in the SGI range */
	ASSERT((uint32_t)(sgi_num) < MIN_PPI_ID);
...
void gicv3_raise_sgi_cpumask(int sgi_num, uint32_t cpumask)
{
	uint32_t cpu = 0;
	uint64_t aff;
	...
	while (cpumask) {
		cpu = ffs32(cpumask);
		aff = cpuinfo_list[cpu].hwid;
		gicv3_raise_sgi(sgi_num, aff);
		cpumask = cpumask & (~(1U << cpu));
	}
}
...
void gicv3_set_spi_routing(unsigned int id, unsigned int irm, u_register_t mpidr)
{
	...
	if (irm == GICV3_IRM_ANY) {
		...
		if (((router >> IROUTER_IRM_SHIFT) & IROUTER_IRM_MASK) == 0U) {
			...
			panic("%s: GICv3 implementation doesn't support routing ANY\n", __func__, id);
		}
	}
}
 ...
 ******************************************************************************/
void gicv3_clear_interrupt_pending(unsigned int id)
{
	uintptr_t gicd_base;
 ...
 * corresponding Redistributor interface.
 ******************************************************************************/
void gicv3_set_interrupt_pending(unsigned int id)
{
	uintptr_t gicd_base;
 ...
 * triggered
 ******************************************************************************/
void gicv3_interrupt_set_cfg(unsigned int id, unsigned int cfg)
{
	gicd_set_icfgr(gicv3_driver_data->gicd_base, id, cfg);
}
