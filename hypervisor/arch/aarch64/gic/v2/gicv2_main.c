...
static const gicv2_driver_data_t _gic_data = {
	.gicd_base = CONFIG_GICD_BASE, //0x08000000,
	.gicc_base = CONFIG_GICC_BASE, //0x08010000,
};
...
gic_ops_t gicv2_ops = {
	.init = gicv2_init,
	.ack_irq = gicv2_acknowledge_interrupt,
	.raise_sgi = gicv2_raise_sgi,
	.enable_irq = gicv2_enable_interrupt,
	.disable_irq = gicv2_disable_interrupt,
	.set_irq_pending = gicv2_set_interrupt_pending,
	.clear_irq_pending = gicv2_clear_interrupt_pending,
	.set_irq_cfg = gicv2_interrupt_set_cfg,
	.set_irq_priority = gicv2_set_interrupt_priority,
	.end_irq = gicv2_end_of_interrupt,
	.deactivate_irq = gicv2_deactivate_interrupt,
};
...
{
	gic_register_ops(&gicv2_ops);
}

void gicv2_init(uint16_t pcpu_id)
{
	if (pcpu_id == 0) {
		gicv2_driver_init(&_gic_data);
		gicv2_distif_init();
...
void gicv2_cpuif_enable(void)
{
	 ...
	 * bypass.
	 */
	val = CTLR_ENABLE_G1_BIT | EOI_MODE_NS;
...
void gicv2_pcpu_distif_init(void)
{
	...
	gicv2_ppi_sgi_setup(driver_data->gicd_base);

	/* Enable G1 interrupts if not already */
	ctlr = gicd_read_ctlr(driver_data->gicd_base);
	if ((ctlr & CTLR_ENABLE_G1_BIT) == 0U) {
		gicd_write_ctlr(driver_data->gicd_base,
						ctlr | CTLR_ENABLE_G1_BIT);
	}
}
...
void gicv2_driver_init(const gicv2_driver_data_t *plat_driver_data)
{
	...
	flush_dcache_range((uintptr_t) driver_data, sizeof(*driver_data));
#endif
	pr_info("ARM GICv2 driver initialized.");
}
...
void gicv2_deactivate_interrupt(unsigned int id)
{
	ASSERT(driver_data != NULL);
	ASSERT(driver_data->gicc_base != 0U);

	//gicc_write_IAR(driver_data->gicc_base, id);
	gicc_write_dir(driver_data->gicc_base, id);
}
 ...
 * system.
 ******************************************************************************/
void gicv2_raise_sgi(int sgi_num, uint32_t cpumask)
{
	...
	sgir_val = GICV2_SGIR_VALUE(SGIR_TGT_SPECIFIC, cpumask, true, sgi_num);
