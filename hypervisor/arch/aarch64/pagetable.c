 ...
 *
 * @pre: level could only PGT_PDPT or PGT_PD
 */
							 ...
							 uint64_t vaddr, const struct pgtable *table)
{
	...
	dev_dbg(DBG_LEVEL_MMU, "%s, paddr: 0x%lx, pbase: 0x%lx", __func__, ref_paddr, pbase);
							  ...
							  uint64_t prot_set, uint64_t prot_clr, const struct pgtable *table, uint32_t type)
{
	...
	dev_dbg(DBG_LEVEL_MMU, "%s, vaddr: [0x%lx - 0x%lx]", __func__, vaddr, vaddr_end);
	for (; index < PTRS_PER_PTE; index++) {
		...
		if (!pgentry_present(table, (*pte))) {
			...
			if ((type == MR_MODIFY) && (vaddr >= MEM_1M)) {
				pr_warn("%s, vaddr: 0x%lx pte is not present.", __func__, vaddr);
			}
		} else {
							  ...
							  uint64_t prot_set, uint64_t prot_clr, const struct pgtable *table, uint32_t type)
{
	...
	for (; index < PTRS_PER_PDE; index++) {
		...
		if (!pgentry_present(table, (*pde))) {
			if (type == MR_MODIFY) {
				pr_warn("%s, addr: 0x%lx pde is not present.", __func__, vaddr);
			}
		} else {
			if (pde_large(*pde) != 0UL) {
				if ((vaddr_next > vaddr_end) || (!mem_aligned_check(vaddr, PDE_SIZE))) {
					split_large_page(pde, PGT_PD, vaddr, table);
				} else {
		...
		if (!pgentry_present(table, (*pdpte))) {
			...
			if (pdpte_large(*pdpte) != 0UL) {
				if ((vaddr_next > vaddr_end) ||
					...
					split_large_page(pdpte, PGT_PDPT, vaddr, table);
				} else {
					...
					uint64_t prot, const struct pgtable *table)
{
	...
	for (; index < PTRS_PER_PTE; index++) {
		...
		if (pgentry_present(table, (*pte))) {
			pr_fatal("%s, pte 0x%lx is already present!", __func__, vaddr);
		} else {
			set_pgentry(pte, paddr | prot | (PAGE_PAGE & PAGE_TYPE_MSK), table);
		}
		paddr += PTE_SIZE;
		vaddr += PTE_SIZE;
					...
					uint64_t prot, const struct pgtable *table)
{
	...
	for (; index < PTRS_PER_PDE; index++) {
		...
		if (pde_large(*pde) != 0UL) {
			...
			if (!pgentry_present(table, (*pde))) {
				if (table->large_page_support(PGT_PD, prot) &&
					mem_aligned_check(paddr, PDE_SIZE) &&
					mem_aligned_check(vaddr, PDE_SIZE) &&
					(vaddr_next <= vaddr_end)) {
					table->tweak_exe_right(&local_prot);
					set_pgentry(pde, paddr | (local_prot | (PAGE_BLOCK & PAGE_TYPE_MSK)), table);
					if (vaddr_next < vaddr_end) {
					  ...
					  uint64_t prot, const struct pgtable *table)
{
	...
	dev_dbg(DBG_LEVEL_MMU, "%s, paddr: 0x%lx, vaddr: [0x%lx - 0x%lx]", __func__, paddr, vaddr, vaddr_end);
	for (; index < PTRS_PER_PDPTE; index++) {
		uint64_t *pdpte = pdpt_page + index;
		uint64_t vaddr_next = (vaddr & PDPTE_MASK) + PDPTE_SIZE;
		...
		if (pdpte_large(*pdpte) != 0UL) {
			...
			if (!pgentry_present(table, (*pdpte))) {
				if (table->large_page_support(PGT_PDPT, prot) &&
					mem_aligned_check(paddr, PDPTE_SIZE) &&
					mem_aligned_check(vaddr, PDPTE_SIZE) &&
					(vaddr_next <= vaddr_end)) {
					table->tweak_exe_right(&local_prot);
					set_pgentry(pdpte, paddr | (local_prot | PAGE_BLOCK), table);
					if (vaddr_next < vaddr_end) {
						paddr += (vaddr_next - vaddr);
						vaddr = vaddr_next;
						continue;
					 ...
					 uint64_t size, uint64_t prot, const struct pgtable *table)
{
	...
	dev_dbg(DBG_LEVEL_MMU, "%s, paddr 0x%lx, vaddr 0x%lx, size 0x%lx", __func__, paddr_base, vaddr_base, size);
