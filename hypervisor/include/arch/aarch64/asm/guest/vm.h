...
#ifndef __AARCH64_VM_H__
#define __AARCH64_VM_H__
...
struct vm_arch {
	...
	struct acrn_vgic vgic;  /* Virtual GIC */
...
static inline struct acrn_vcpu *vm_get_vcpu_by_mpidr(struct acrn_vm *vm, uint64_t mpidr)
{
	uint16_t vcpu_id = mpidr_to_cpu_id(mpidr);
